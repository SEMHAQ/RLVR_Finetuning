...
所有实验使用TRL框架的GRPOTrainer，主要配置如表~\ref{tab:config}所示。
...
微调方式 & LoRA (r=16, alpha=32, target\_modules=q\_proj,k\_proj,v\_proj,o\_proj) \\
学习率 & 2e-5（主实验）/ 消融实验见表~\ref{tab:ablation\_lr} \\
Batch Size & 6 \\
Num Generations & 3 \\
Max Completion Length & 256 tokens \\
训练轮数 & 1 epoch (934 steps) \\
优化器 & AdamW \\
学习率调度 & Cosine \\
硬件 & RTX 3090 24GB \\
\bottomrule
...
表~\ref{tab:main}展示了GRPO在GSM8K和MATH-500上的主要结果。
...
表~\ref{tab:lora}对比了全量微调与LoRA的效果。

\begin{table}[H]
\centering
\caption{全量微调 vs LoRA}
\label{tab:lora}
\begin{tabular}{lccc}
\toprule
...
表~\ref{tab:msf}展示了MSF-GRPO与纯规则GRPO的对比。
